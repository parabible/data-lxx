...
fs.readdir(mlxxRoot, (err, filenames) => {
	...
	completeProcess()
	console.log("Writing data/allwords.json")
	fs.writeFileSync("./data/allwords.json", JSON.stringify(allWords, null, 2), 'utf8')
})
...
const parseFile = (lines, filename) => {
	...
	for (i in lines) {
		...
		if (+i === 5558 && filename === "DanielOG") {
			...
			verse_nr = 31 //That's what it is in the lemmas file
			word_nr = -1
		}
		else if (line === "Bel 15-17") {
			verse_nr = 15 //That's what it is in the lemmas file
			word_nr = -1
		}
			...
			if (Object.keys(exceptions).includes(line)) {
				...
				chapter_nr = exceptions[k]["ch"]
				verse_nr = exceptions[k]["v"]
			}
			word_nr = -1
		}
		else if (/.+ .+:.+/.test(line)) { // Ruth 1:1
			const lineParts = line.split(/[\s|:]/)
			book_name = lineParts[0]
			chapter_nr = lineParts[1]
			verse_nr = lineParts[2]
			word_nr = -1
		}
			...
			let secondaryKey = `${book_name}.${chapter_nr}.${verse_nr}`
			if (!lemmas[primaryKey][secondaryKey])
				secondaryKey = `${bookMap[book_name]}.${chapter_nr}.${verse_nr}`

			let extradetails = {}
			if (!lemmas[primaryKey][secondaryKey]) {
				fails.push(`(${filename})[${primaryKey}] (${book_name})[${secondaryKey}] [${word_nr}] + ${i}:${word}`)
				oth = null
				return
			}
			else {
				extradetails = lemmas[primaryKey][secondaryKey][word_nr]
			}

			if (!extradetails || !extradetails.hasOwnProperty("lemma")) {
				console.log(primaryKey, secondaryKey, word_nr, word)
			}
			const wordDetails = {
				id: count,
				book_name,
